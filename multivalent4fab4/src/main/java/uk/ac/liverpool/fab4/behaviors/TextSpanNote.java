 ...
 * 
 */
public class TextSpanNote extends Fab4LabelSpan {
	...
	public boolean semanticEventAfter(SemanticEvent se, String msg) {
		if (this != se.getIn()) {
			...
			if (map != null && map.get("cancel") == null) {
				String val = (String) map.get(TextSpanNote.ATTR_TEXT);
				if (val != null)
					theText = val;
				putAttr(TextSpanNote.ATTR_TEXT, theText);
				return true;
			}
		}
	...
	public boolean paintBefore(Context cx, Node start) {
		if (visible) {
			...
			g.setComposite(cc);
			TextSpanNote.annoFont.drawString(g, theText, cx.x - 100,
					TextSpanNote.annoFont.getAscent() - 100);
			g.setClip(r);
			g.setComposite(c);
			return false;
		} else
	...
	public void restore(ESISNode n, Map<String, Object> attr, Layer layer) {
		...
		theText = getAttr(TextSpanNote.ATTR_TEXT, "(nothing)");
		putAttr(TextSpanNote.ATTR_TEXT, theText);
