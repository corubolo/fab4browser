...
public final class Line extends SimpleShape {
    ...
    */
    ...
    public int getLineEndArrowHead(){
        ...
        EscherSimpleProperty prop = (EscherSimpleProperty)getEscherProperty(opt, EscherProperties.LINESTYLE__LINEENDARROWHEAD);
        int width = prop == null ? 0 : prop.getPropertyValue();
        return width;
    }

    public int getLineEndArrowLenght(){
        EscherOptRecord opt = (EscherOptRecord)getEscherChild(_escherContainer, EscherOptRecord.RECORD_ID);
        EscherSimpleProperty prop = (EscherSimpleProperty)getEscherProperty(opt, EscherProperties.LINESTYLE__LINEENDARROWLENGTH);
        int width = prop == null ? 0 : prop.getPropertyValue();
        return width;
    }
    public int getLineEndArrowWidth(){
        EscherOptRecord opt = (EscherOptRecord)getEscherChild(_escherContainer, EscherOptRecord.RECORD_ID);
        EscherSimpleProperty prop = (EscherSimpleProperty)getEscherProperty(opt, EscherProperties.LINESTYLE__LINEENDARROWWIDTH);
        int width = prop == null ? 0 : prop.getPropertyValue();
        return width;
    }
    public int getLineStartArrowHead(){
        EscherOptRecord opt = (EscherOptRecord)getEscherChild(_escherContainer, EscherOptRecord.RECORD_ID);
        EscherSimpleProperty prop = (EscherSimpleProperty)getEscherProperty(opt, EscherProperties.LINESTYLE__LINESTARTARROWHEAD);
        int width = prop == null ? 0 : prop.getPropertyValue();
        return width;
    }
    public int getLineStartArrowWidth(){
        EscherOptRecord opt = (EscherOptRecord)getEscherChild(_escherContainer, EscherOptRecord.RECORD_ID);
        EscherSimpleProperty prop = (EscherSimpleProperty)getEscherProperty(opt, EscherProperties.LINESTYLE__LINESTARTARROWWIDTH);
        int width = prop == null ? 0 : prop.getPropertyValue();
        return width;
    } public int getLineStartArrowLenght(){
        EscherOptRecord opt = (EscherOptRecord)getEscherChild(_escherContainer, EscherOptRecord.RECORD_ID);
        EscherSimpleProperty prop = (EscherSimpleProperty)getEscherProperty(opt, EscherProperties.LINESTYLE__LINESTARTARROWLENGTH);
        int width = prop == null ? 0 : prop.getPropertyValue();
        return width;
    }
