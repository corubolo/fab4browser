...
public class LibJpeg2kDecoder extends ImageReader{

	BufferedImage img = null;

	public LibJpeg2kDecoder(LibJPEG2kLLJVMReaderSPI libJPEGNestedReaderSPI, Object extension) {
	...
	BufferedImage decode () throws IOException{
		...
		File out;
		out = File.createTempFile("multv", ".ppm");
		out.deleteOnExit();
		File in = File.createTempFile("multv", ".j2k");
		in.deleteOnExit();
...
//		(ImageInputStream)input).close();
		os.close();
		j2k_to_image.main(new String[]{"j2k_to_image", "-i " + in.getAbsolutePath(),"-o " + out.getAbsolutePath()});
...
//		int status = me.run("djpeg", args);
		...
		System.out.println("Libjpeg-2K-lljvm decoded " + img);
		return img;
	}
