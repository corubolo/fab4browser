...
public class CharsetDetector {
	...
	public CharsetDetector setText(InputStream in) throws IOException {
		fInputStream = in;
		fInputStream.mark(CharsetDetector.kBufSize);
		fRawInput = new byte[CharsetDetector.kBufSize];   // Always make a new buffer because the
		//   previous one may have come from the caller,
		//   in which case we can't touch it.
		fRawLength = 0;
		int remainingLength = CharsetDetector.kBufSize;
	...
	public CharsetMatch[] detectAll() {
		...
		for (i=0; i<CharsetDetector.fCSRecognizers.size(); i++) {
			csr = (CharsetRecognizer)CharsetDetector.fCSRecognizers.get(i);
	...
	private void MungeInput() {
		...
		if (openTags<5 || openTags/5 < badTags ||
			...
			if (limit > CharsetDetector.kBufSize)
				limit = CharsetDetector.kBufSize;
	...
	byte[]      fInputBytes =       // The text to be checked.  Markup will have been
		new byte[CharsetDetector.kBufSize];  //   removed if appropriate.
	...
	private static ArrayList createRecognizers() {
		...
		CharsetDetector.fCharsetNames = new String[out];
		System.arraycopy(charsetNames, 0, CharsetDetector.fCharsetNames, 0, out);
