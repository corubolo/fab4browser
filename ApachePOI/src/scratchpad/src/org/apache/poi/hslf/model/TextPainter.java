...
import uk.ac.liverpool.font.DingabatMapping;
...
public final class TextPainter {
	...
	public static final AdditionalTextAttribute TEXT_SHADOWED = new AdditionalTextAttribute("shadowed");
	public static final String TEXT_SHADOWED_ON = new String("on");
	public static final String TEXT_SHADOWED_OFF =  new String("off");

	public static class AdditionalTextAttribute extends Attribute {
		/**
		 * 
		 */
		private static final long serialVersionUID = 1L;

		protected AdditionalTextAttribute(final String arg0) {
			super(arg0);
		}
	...
	public AttributedString getAttributedString(TextRun txrun){
		...
		for (int i = 0; i < rt.length; i++) {
			...
			if(rt[i].isShadowed()) at.addAttribute(TEXT_SHADOWED, TEXT_SHADOWED_ON, start, end);
	...
	public void paint(Graphics2D graphics){
		...
		for (int i = 0; i < elem.length; i++) {
			...
			// compute if the string is shadowed, and the shadow color
			boolean shadowed = false;
			Color c = Color.gray;
			Color ct = Color.black;
			AttributedCharacterIterator chIt = elem[i]._text.getIterator();
			if(chIt.getEndIndex() > chIt.getBeginIndex()) {
				if (chIt.getAllAttributeKeys().contains(TEXT_SHADOWED)){
					if (chIt.getAttribute(TEXT_SHADOWED)!=null) {
						if (chIt.getRunLimit(TEXT_SHADOWED)>=chIt.getEndIndex()){
							shadowed = true;
							ct = (Color)chIt.getAttribute(TextAttribute.FOREGROUND);
							if ((ct.getRed() + ct.getBlue() + ct.getGreen()) > (c.gray.getRed() + c.gray.getBlue() + c.gray.getGreen()))
								c = c.black;
							else c = c.gray;
						}
					}
			...
			// draws the bullet and shadow 
			if(elem[i]._bullet != null){
				AttributedCharacterIterator a = (AttributedCharacterIterator)elem[i]._bullet.getIterator().clone();
				AttributedString at = new AttributedString(a);
				if (shadowed){
					Color c2 = (Color)a.getAttribute(TextAttribute.FOREGROUND);
					at.addAttribute(TextAttribute.FOREGROUND, c);
					graphics.drawString(at.getIterator(), (float)(pen.x + elem[i]._bulletOffset)+1, (float)pen.y+1);
					if (c2 == null) {
						at.addAttribute(TextAttribute.FOREGROUND, ct);
					}
				}
				graphics.drawString(at.getIterator(), (float)(pen.x + elem[i]._bulletOffset), (float)pen.y);
			}
			// draws the text
			if(chIt.getEndIndex() > chIt.getBeginIndex()) {
				if (shadowed){
					AttributedCharacterIterator a = (AttributedCharacterIterator)chIt.clone();
					AttributedString at = new AttributedString(a);
					at.addAttribute(TextAttribute.FOREGROUND, c);
					graphics.drawString(at.getIterator(), (float)(pen.x + elem[i]._textOffset)+1, (float)pen.y+1);
				}
	...
	public TextElement[] getTextElements(float textWidth, FontRenderContext frc){
		...
		while (measurer.getPosition() < paragraphEnd) {
			...
			if(rt.isBullet() && (prStart || startIndex == 0)){
				...
				AttributedString bat;
				char bulletChar;
				// Empiric method to verify if it is a valid unicode bullet character, or a wrongly mapped to a latin character bullet
				if (rt.getBulletChar()<0x2000)
					bulletChar = (char)(0xF000 + rt.getBulletChar());
				else
					bulletChar = rt.getBulletChar();

				bat = new AttributedString(Character.toString(bulletChar));
				...
				//System.out.println("Bullet: "+ Character.toString(rt.getBulletChar()) +" - "+ bulletFont.getFontName() + (int)rt.getBulletChar());
				...
				if(!new Font(bulletFont.getFontName(), Font.PLAIN, 1).canDisplay(bulletChar)){
					//                    bat.addAttribute(TextAttribute.FAMILY, "Arial");
					//                    bat = new AttributedString("" + DEFAULT_BULLET_CHAR, bat.getIterator().getAttributes());
					bat.addAttribute(TextAttribute.FONT, DingabatMapping.getUnicodeFont());
					bat = new AttributedString("" + DingabatMapping.toUnicode(bulletChar,bulletFont.getFontName()), bat.getIterator().getAttributes());
					//System.out.println("bullett replaces");
				}
